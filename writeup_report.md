...
My model is essentially the same model as the one show in section 15 (Even More powerful network) of the proyect chapter  ( function `build_model` in `model.py lines` 152-176) 
...
As mentioned above, the model contains dropout layers in order to reduce overfitting (function `build_model` in `model.py` lines 21). 
